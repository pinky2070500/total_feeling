...
use app\modules\quanly\models\capnuocgd\GdOngcaiSearch;
...
/**
 * GdOngcaiController implements the CRUD actions for GdOngcai model.
 */
class GdOngcaiController extends QuanlyBaseController
{
    ...
    {
        $searchModel = new GdOngcaiSearch();
    ...
    /**
     * Displays a single GdOngcai model.
     * @param integer $id
     * @return mixed
     */
    ...
    public function actionCreate()
    {
        ...
        if ($model->load($request->post()) && $model->save()) {
            Yii::$app->db->createCommand("UPDATE gd_ongcai SET geom = ST_SETSRID(ST_GeomFromText(ST_AsText(ST_GeomFromGeoJSON('" . $model->geojson . "'))),4326) WHERE id = :id")
    ...
    /**
     * Finds the GdOngcai model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return GdOngcai the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = GdOngcai::findOne($id)) !== null) {
            return $model;
        } else {
